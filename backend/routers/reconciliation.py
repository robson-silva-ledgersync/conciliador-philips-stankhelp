        ...
        records=[
            {
                ...
                "outras_despesas": float(r.outras_despesas) if r.outras_despesas is not None else None,
                "quilometragem": float(r.quilometragem) if r.quilometragem is not None else None,
                "hospedagem": float(r.hospedagem) if r.hospedagem is not None else None,
                "reembolso_total": float(r.reembolso_total) if r.reembolso_total is not None else None,
                "mdo": float(r.mdo) if r.mdo is not None else None,
                "tecnico": r.tecnico,
                "contrato": r.contrato,
...
def export_reconciliation(
    ...
    for rec in recon.records:
        row = {
            ...
            "Distância KM": float(rec.distancia_km) if rec.distancia_km is not None else "",
            "Outras Despesas": float(rec.outras_despesas) if rec.outras_despesas is not None else "",
            "Quilometragem": float(rec.quilometragem) if rec.quilometragem is not None else "",
            "Hospedagem": float(rec.hospedagem) if rec.hospedagem is not None else "",
            "Reembolso Total": float(rec.reembolso_total) if rec.reembolso_total is not None else "",
            "MDO (R$)": float(rec.mdo) if rec.mdo is not None else "",
            "Técnico": rec.tecnico,
